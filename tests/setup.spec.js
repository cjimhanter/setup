...
test('English setup loads complete, local images and honest component statuses', async ({ page }) => {
  const errors = [];
  ...
  await expect(page.locator('#pc-list .status-planned')).toHaveCount(3);
  await expect(page.locator('#pc-list .status-owned')).toHaveCount(4);
  await expect(page.locator('.spec-row').filter({ hasText: 'RX 6600' })).toContainText('Planned');
  await expect(page.locator('#case-card')).toContainText('Not confirmed');
...
for (const failure of ['http', 'json', 'schema', 'network', 'timeout']) {
  test(`${failure} failure has a retry and does not disable software details`, async ({ page }) => {
    ...
    await page.route('**/data/setup.json', async route => {
      ...
      return route.fulfill({ json: { ...data, pc: [{ ...data.pc[0], status: 'wrong' }] } });
    });
